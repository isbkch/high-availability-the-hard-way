#!/usr/bin/env bash
# Run smoke tests against the idempotency lab.
...
log_info "Running Lab 4 smoke tests against $API_URL"
...
if [[ "$STATUS" != "healthy" ]]; then
    log_error "Expected idempotency lab health status healthy, got $STATUS: $HEALTH"
    exit 1
fi
...
DOC_RESPONSE="$(
    curl -fsS -X POST "$API_URL/api/documents" \
        ...
        -d '{"title":"Idempotency Smoke Test Doc","content":"Idempotency smoke tests verify upload, worker embedding, and Q&A through the mock LLM."}'
)"
...
QUESTION_RESPONSE="$(
    curl -fsS -X POST "$API_URL/api/questions" \
        -H "Content-Type: application/json" \
        -d "{\"question\":\"What do idempotency smoke tests verify?\",\"document_id\":$DOC_ID}"
)"
...
log_info "All Lab 4 smoke tests passed"
