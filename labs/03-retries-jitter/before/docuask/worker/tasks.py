"""Dramatiq tasks for document processing with immediate embedding retries."""
...
async def embed_texts(texts: list[str]) -> list[list[float]]:
    """Call the LLM embeddings endpoint with immediate bounded retries."""
        ...
        for attempt in range(1, MAX_EMBEDDING_RETRY_ATTEMPTS + 1):
            try:
                ...
                if attempt == MAX_EMBEDDING_RETRY_ATTEMPTS:
                    raise RuntimeError(
                        "LLM embeddings failed after immediate retry attempts"
                    ) from exc
                continue

    if last_response is not None:
        last_response.raise_for_status()
    raise RuntimeError("LLM embeddings failed") from last_error
