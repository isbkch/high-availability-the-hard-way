"""OpenAI-compatible LLM client with immediate retries.

This is intentionally bad for Lab 3: retries are bounded, but each retry is
sent immediately. Under intermittent 503s, many callers align into a retry
storm and amplify load on the dependency.
"""
...
class LLMClient:
    ...
    async def _post_with_immediate_retries(self, path: str, payload: dict) -> httpx.Response:
            ...
            for attempt in range(1, MAX_RETRY_ATTEMPTS + 1):
                try:
                    ...
                    if attempt == MAX_RETRY_ATTEMPTS:
                        raise RuntimeError(
                            f"LLM request failed after {MAX_RETRY_ATTEMPTS} immediate attempts"
                        ) from exc
                    continue
    ...
    async def embed_texts(self, texts: list[str]) -> list[list[float]]:
        ...
        response = await self._post_with_immediate_retries("/embeddings", payload)
        data = response.json()
    ...
    async def answer_question(self, question: str, contexts: list[str]) -> str:
        ...
        response = await self._post_with_immediate_retries("/chat/completions", payload)
        data = response.json()
