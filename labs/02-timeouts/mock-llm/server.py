"""Tiny OpenAI-compatible mock server for Lab 2."""
...
def embedding_for(text: str) -> list[float]:
    ...
    return [
        float((total % 97) / 97),
        float((length % 89) / 89),
        float((text.lower().count("t") % 13) / 13),
    ]
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/v1/models":
            self.send_json(
                ...
                {
                    "object": "list",
                    "data": [{"id": "timeout-mock", "object": "model"}],
                },
            )
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path == "/v1/chat/completions":
            ...
            self.send_json(
                200,
                {
                    "id": "chatcmpl-timeouts",
                    "object": "chat.completion",
                    "model": payload.get("model", "timeout-mock"),
                    "choices": [
                        {
                            "index": 0,
                            "message": {
                                "role": "assistant",
                                "content": (
                                    "Timeout lab mock answer based on supplied "
